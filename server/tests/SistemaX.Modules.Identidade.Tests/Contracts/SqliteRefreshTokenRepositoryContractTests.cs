...
public sealed class SqliteRefreshTokenRepositoryContractTests : RefreshTokenRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"financemax-refresh-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteRefreshTokenRepositoryContractTests()
    {
    ...
    protected override IRefreshTokenRepository CriarRepositorio()
        => new SqliteRefreshTokenRepository(_connectionFactory, new SessaoSempreInativa());
