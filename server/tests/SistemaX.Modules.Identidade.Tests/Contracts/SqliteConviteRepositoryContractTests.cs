...
public sealed class SqliteConviteRepositoryContractTests : ConviteRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"financemax-convite-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteConviteRepositoryContractTests()
    {
        ...
        new IdentidadeSchemaMigrationV3().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IConviteRepository CriarRepositorio()
        => new SqliteConviteRepository(_connectionFactory, new SessaoSempreInativa());
